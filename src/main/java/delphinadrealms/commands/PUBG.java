...
public class PUBG {
        ...
        if (!mode1.equals(null) && !region1.equals(null)) {
            try {
                ...
                if (e.getMessage().contains("no matches played")) {
                    channel.sendMessageFormat("User %s doesn't exist or has no matches played.",username).queue();
                } else {
                    e.printStackTrace();
                }
    ...
    private static String[] getStats(Player player) {
        ...
        try {
             ...
             // 0= RATING 1= KillsPer game 2= AvgDmg 3 = Wins 4 =
             return Stats;
         } catch (ApiException e) {
             e.printStackTrace();
             String[] Stats = {"error"};
